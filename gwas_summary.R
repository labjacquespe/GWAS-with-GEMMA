...
top_hits = top_hits[order(top_hits$qval), ]$rs
...
extract_geno_command = paste0("plink2 --bfile data/dataset --extract ",gemma_prefix,".tophits.txt --recode A-transpose --out ", gemma_prefix,".tophits")
...
for (hit in top_top_hits){
	df[, hit] = ifelse(df[, hit]==2, 0, df[, hit])
	df[, hit] = ifelse(df[, hit]==1, 2, df[, hit])
	print(ggplot(df, aes(x = as.factor(df[, hit]), y = estimate, fill = as.factor(df[, hit]))) + 
		geom_boxplot(width=0.5, color="grey", outlier.shape = NA) + labs(x = hit, y = "Phenotype") + 
		scale_colour_manual(values = cols, aesthetics = c("colour", "fill")) + geom_jitter(width=0.3, shape=0) + 
